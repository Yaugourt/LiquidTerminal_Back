...
/**
 * Client for HypeDexer Builders API
 * GET /builders/?limit=1000
 * Fetches all builders at once (< 1000 total) and caches in Redis.
 * Service layer handles pagination, filtering and sorting.
 */
export class HLIndexerBuildersClient extends BaseApiService {
  ...
  private constructor() {
    super(HLIndexerBuildersClient.API_URL, {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'X-API-Key': HLIndexerBuildersClient.API_KEY
    });
  ...
  public startPolling(): void {
    ...
    this.updateBuildersData().catch(err =>
      logDeduplicator.error('Error in initial Builders update:', { error: err instanceof Error ? err.message : String(err) })
    );
    this.pollingInterval = setInterval(() => {
      this.updateBuildersData().catch(err =>
        logDeduplicator.error('Error in Builders polling:', { error: err instanceof Error ? err.message : String(err) })
      );
    }, UPDATE_INTERVAL);
  }
  ...
  private async fetchAllBuilders(): Promise<BuildersApiResponse> {
    return this.circuitBreaker.execute(() =>
      this.get<BuildersApiResponse>('/builders/?limit=1000&offset=0&sort=volume_usd&order=DESC')
    );
  }
